...
Local JWT authentication is implemented: `backend/app/deps.py::get_current_user_id()` validates the `Authorization: Bearer` header (HS256, 7-day expiry) and returns the signed-in user's id — every endpoint in every router depends on it. Hashing (`bcrypt`) and JWT (`python-jose`) utilities live in `app/auth.py`; registration/login/`me` are in `app/routers/auth.py`. There is no seed user anymore — `app/seed.py` was removed when auth landed, and users start with an empty account after registering. AWS Cognito (Phase 4) will replace this, swapping only how `deps.py` verifies the token.
